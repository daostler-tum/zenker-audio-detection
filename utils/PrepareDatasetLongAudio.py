"""
This script prepares the long audio files (patient level inference) of the SwallowSet dataset by organizing them into a structured directory format.
...
import librosa
import soundfile
from config import get_raw_data_dir, get_long_audio_dir

# init
count = 0
root_dir = get_raw_data_dir()
save_path = get_long_audio_dir()
...
for cl in classes:
    if cl == "Idle":
        ...
        for specimen in specimens:
            ...
            subfolder = [f for f in subfolder if os.path.isdir(os.path.join(root_dir, cl, specimen, f))]
            subfolder = [f for f in subfolder if "long" in f.lower()]
            try:
            #  print("number of subfolders for specimen: " + specimen_id + " is " + str(len(subfolder)))
                files = os.listdir(os.path.join(root_dir, cl, specimen, subfolder[0]))
                wav_files = [f for f in files if (".wav" in f or ".WAV" in f)]
            except Exception as e:
                print(e)
                print("No long file for specimen: " + specimen + " in class: " + cl)
                continue
            for file in wav_files:
                ...
                # Ensure the file is saved with a lower .wav extension
