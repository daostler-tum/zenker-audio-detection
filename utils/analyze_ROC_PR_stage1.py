...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute ROC/PR curves for Stage 1 AST classifier (Idle vs Swallow) on held-out data."
    )
    parser.add_argument(
        "--data-dir", default="data_ast_stage1", help="Directory with fold npy files."
    )
    parser.add_argument(
        "--model-root-template",
        default="runs/ast_classifier_stage1/fold{fold}/best",
    ...
    with torch.inference_mode():
        for batch_entries in batched(X, batch_size):
            ...
            probs = torch.softmax(logits, dim=1)[
                :, 1
            ]  # probability of Swallow (class 1)
...
def evaluate_fold(
    ...
    for thr in args.decision_thresholds:
        ...
        report = classification_report(
            y_true,
            preds,
            target_names=["Idle", "Swallow"],
            output_dict=True,
            zero_division=0,
        )
        metrics[f"thr_{thr}_confusion"] = cm.tolist()
        metrics[f"thr_{thr}_precision"] = report["Swallow"]["precision"]
        metrics[f"thr_{thr}_recall"] = report["Swallow"]["recall"]
        metrics[f"thr_{thr}_f1"] = report["Swallow"]["f1-score"]
...
def main():
    ...
    if args.plot:
        ...
        if plot_combined and len(reports) > 1:
            ...
            ax.set_title("Stage 1 ROC Curves (Idle vs Swallow) - All Folds")
            ax.legend(loc="lower right", fontsize="small")
            roc_path = os.path.join(args.plot_dir, "stage1_roc_combined.png")
            fig.tight_layout()
            ...
            ax.set_title(
                "Stage 1 Precision-Recall Curves (Idle vs Swallow) - All Folds"
            )
            ax.legend(loc="lower right", fontsize="small")
            pr_path = os.path.join(args.plot_dir, "stage1_pr_combined.png")
            fig.tight_layout()
        ...
        if plot_individual:
            for fold_metrics in reports:
                ...
                ax.set_xlabel("False Positive Rate")
                ax.set_ylabel("True Positive Rate")
                ax.set_title(f"Stage 1 ROC Curve (Idle vs Swallow) - Fold {fold}")
                ax.legend(loc="lower right")
                roc_path = os.path.join(args.plot_dir, f"stage1_roc_fold{fold}.png")
                fig.tight_layout()
                save_figure(fig, roc_path, args.plot_dpi, args.plot_format)
                plt.close(fig)
                ...
                ax.set_ylim(0.0, 1.05)
                ax.set_title(
                    f"Stage 1 Precision-Recall Curve (Idle vs Swallow) - Fold {fold}"
                )
                ax.legend(loc="lower right")
                pr_path = os.path.join(args.plot_dir, f"stage1_pr_fold{fold}.png")
                fig.tight_layout()
                save_figure(fig, pr_path, args.plot_dpi, args.plot_format)
                plt.close(fig)
