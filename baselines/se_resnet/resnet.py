...
def ResNet(
    ...
    if input_shape is None:
        input_shape = (
            (32, 32, 3) if K.image_data_format() == "channels_last" else (3, 32, 32)
        )
    ...
    for i, r in enumerate(repetitions):
        ...
        if use_se:
            se = se_block(block, num_filters=filters)
            block = Add()([block, se])
        filters *= 2
...
def ResNet18(input_shape, classes, use_se=False):
    """ResNet with 18 layers and v2 residual units"""
    return ResNet(
        input_shape, classes, basic_block, repetitions=[2, 2, 2, 2], use_se=use_se
    )
