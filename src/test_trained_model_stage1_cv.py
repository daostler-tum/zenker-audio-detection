...
"""Evaluation script for Stage 1 (Idle vs Swallow) cross-validation models.
Models expected under runs/ast_classifier_stage1/fold{k}/best
Data expected under data_ast_stage1/test_x_fold{k}.npy etc.
Labels: 0 -> Idle, 1 -> Swallow
"""
...
MODEL_ROOT = os.path.join(PROJECT_ROOT, "runs", "ast_classifier_stage1")
SAMPLING_RATE = 16000
CLASS_NAMES = None  # will be loaded dynamically; order: index ascending
DEFAULT_MAPPING_PATH = os.path.join(PROJECT_ROOT, "data_ast_stage1", "class_mapping.json")
    ...
    if os.path.exists(target):
        try:
            ...
            if isinstance(data, dict):
                ...
                print(
                    f"[WARN] Mapping file {target} not a dict; using fallback Idle/Swallow"
                )
        except Exception as e:
            print(f"[WARN] Failed reading class mapping {target}: {e}")
    else:
        print(
            f"[INFO] Class mapping file not found at {target}; using fallback Idle/Swallow"
        )
    return ["Idle", "Swallow"]  # Ensure Idle=0, Swallow=1


# Historical default normalization fallback
...
def evaluate_fold(fold: int, use_aggregate_stats: bool, wandb_run=None):
    ...
    # Also persist classification report into a central results directory
    central_results_dir = os.path.join(PROJECT_ROOT, "results", "stage1")
    ...
    if wandb_run is not None:
        try:
            cm_plot = wandb.plot.confusion_matrix(
                ...
                title=f"Stage1 Fold {fold} Confusion Matrix",
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate Stage1 AST models on CV folds"
    )
    ...
    parser.add_argument(
        "--wandb-group", default="cv-eval-stage1", help="Optional run group"
    )
    parser.add_argument(
        "--class-mapping-path",
        default=None,
    ...
    if args.wandb:
        ...
        run_name = args.wandb_run_name or (
            f"stage1-cv-eval-fold{args.fold}" if args.fold else "stage1-cv-eval-all"
        )
    ...
    # Load class mapping after potential wandb init so we can update config
    global CLASS_NAMES
    ...
    if args.all:
        ...
        with open(os.path.join(agg_dir, "classification_report.txt"), "w") as f:
            f.write(agg_cr + "\n")
        # Write aggregate report to central results directory
        if args.model_root is not None:
            # If custom model root, put results inside it
            central_results_dir = os.path.join(MODEL_ROOT, "results", "stage1")
        else:
        ...
        if wandb_run is not None:
            try:
                agg_plot = wandb.plot.confusion_matrix(
                    ...
                    y_true=all_true,
                    class_names=CLASS_NAMES,
                    title="Stage1 Aggregate Confusion Matrix",
                )
